...
const ImageTintAdjusterPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            <div className="flex items-center gap-4 mb-8">
                <ImageTintAdjusterIcon className="w-10 h-10 text-rose-300" />
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Image Tint Adjuster</h1>
            </div>
            {!imageSrc ? (
                <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-lg">
                    <button onClick={() => fileInputRef.current?.click()} className="px-6 py-3 bg-rose-300 text-white font-bold rounded-lg hover:bg-rose-400 transition shadow-lg text-lg">
                        Select Image
                    </button>
                    ...
                    <div>
                        ...
                        <div className="p-4 bg-white rounded-lg shadow-md border space-y-4">
                            <div>
                                ...
                                <input type="color" value={tintColor} onChange={(e) => setTintColor(e.target.value)} className="w-full h-10 mt-1 p-1 border rounded-md" />
                            </div>
                            <div>
                                <label className="block font-medium">Intensity: {Math.round(intensity*100)}%</label>
                                <input type="range" min="0" max="1" step="0.05" value={intensity} onChange={(e) => setIntensity(parseFloat(e.target.value))} className="w-full mt-1 accent-rose-300" />
                            </div>
...
export default ImageTintAdjusterPage;
