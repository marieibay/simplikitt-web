...
const ImagePixelateFilterPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (ctx) {
            // Draw original image small for better performance, then scale up
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            <div className="flex items-center gap-4 mb-8">
                <ImagePixelateFilterIcon className="w-10 h-10 text-gray-600" />
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Image Pixelate Filter</h1>
            </div>

            {!imageSrc ? (
                <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-lg">
                    <button onClick={() => fileInputRef.current?.click()} className="px-6 py-3 bg-gray-600 text-white font-bold rounded-lg hover:bg-gray-700 transition shadow-lg text-lg">
                        Select Image
                    </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div>
                        ...
                        <div className="p-4 bg-white rounded-lg shadow-md border space-y-4">
                            <div>
                                <label className="block font-medium">Pixel Size: {pixelSize}</label>
                                <input type="range" min="2" max="50" value={pixelSize} onChange={(e) => setPixelSize(parseInt(e.target.value))} className="w-full mt-1 accent-gray-600" />
                            </div>
...
export default ImagePixelateFilterPage;