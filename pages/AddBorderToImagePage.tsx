...
const AddBorderToImagePage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            <div className="flex items-center gap-4 mb-8">
                <AddBorderToImageIcon className="w-10 h-10 text-stone-500" />
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Add Border to Image</h1>
            </div>

            {!imageSrc ? (
                <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-lg">
                    <button onClick={() => fileInputRef.current?.click()} className="px-6 py-3 bg-stone-500 text-white font-bold rounded-lg hover:bg-stone-600 transition shadow-lg text-lg">
                        Select Image
                    </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        ...
                        <div className="p-4 bg-gray-100 rounded-lg border flex items-center justify-center min-h-[400px]">
                            <canvas ref={canvasRef} className="max-w-full max-h-[500px] shadow-lg"></canvas>
                        </div>
                    </div>
                    <div>
                        ...
                        <div className="p-4 bg-white rounded-lg shadow-md border space-y-4">
                            <div>
                                <label className="block font-medium">Border Width: {borderWidth}px</label>
                                <input type="range" min="1" max="100" value={borderWidth} onChange={(e) => setBorderWidth(parseInt(e.target.value))} className="w-full mt-1 accent-stone-500" />
                            </div>
                            <div>
                                <label className="block font-medium">Border Color</label>
                                <input type="color" value={borderColor} onChange={(e) => setBorderColor(e.target.value)} className="w-full h-10 mt-1 p-1 border rounded-md" />
                            </div>
...
export default AddBorderToImagePage;