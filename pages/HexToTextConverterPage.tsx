...
const HexToTextConverterPage: React.FC = () => {
  ...
  const handleConvert = () => {
    ...
    if (hexString.length % 2 !== 0) {
        setError('Invalid hex string length.');
        return;
    }
    try {
        let text = '';
        for (let i = 0; i < hexString.length; i += 2) {
            ...
            if (!/^[0-9a-fA-F]{2}$/.test(hexByte)) {
                throw new Error('Invalid hex character detected.');
            }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="flex items-center gap-4 mb-8">
        <HexToTextConverterIcon className="w-10 h-10 text-slate-500" />
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Hex to Text Converter</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label htmlFor="input-text" className="block text-sm font-medium text-gray-700 mb-1">Input Hex</label>
          <textarea
            id="input-text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Enter hex string, with or without spaces..."
            className="w-full h-80 p-3 font-mono border border-gray-300 rounded-lg shadow-sm focus:ring-slate-500 focus:border-slate-500 transition"
          />
        </div>
        <div>
      ...
      <div className="mt-6 flex flex-wrap gap-3 items-center">
        <button onClick={handleConvert} className="px-5 py-2.5 bg-slate-500 text-white font-bold rounded-lg hover:bg-slate-600 transition shadow-md">
          Convert
        </button>
...
export default HexToTextConverterPage;