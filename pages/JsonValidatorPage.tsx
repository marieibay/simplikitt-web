  ...
  const handleValidate = () => {
    ...
    try {
      ...
      if (e instanceof Error) {
        setResult({ status: 'invalid', message: `Invalid JSON: ${e.message}` });
      } else {
        setResult({ status: 'invalid', message: 'An unknown error occurred while parsing JSON.' });
      }
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="flex items-center gap-4 mb-6">
        <JsonValidatorIcon className="w-10 h-10 text-blue-500" />
      ...
      <textarea
        ...
        className={`w-full h-96 p-3 font-mono text-sm border rounded-lg shadow-sm transition focus:ring-2 ${result?.status === 'invalid' ? 'border-red-500 focus:ring-red-300' : 'border-gray-300 focus:ring-blue-300 focus:border-blue-500'}`}
      />
      
      {result && <div className={`mt-4 p-4 rounded-lg font-semibold ${resultClasses[result.status]}`}>{result.message}</div>}
      
      <div className="mt-6 flex gap-4">
        <button onClick={handleValidate} className="px-6 py-2 bg-blue-500 text-white font-bold rounded-lg hover:bg-blue-600 transition shadow-md">
          Validate JSON
        </button>
